if event.getPreviousValue() != None:
	...
	spreader_tw_locked_value = newValue.getValue()
	tag_path = event.getTagPath().toString()
	rtg_name = tag_path.split("/")[3]
	logger = system.util.getLogger('Job Stepping Tag_RTG_TT_Lift (Tag Change)')
	...
	# Verify if we are doing a lift
	if spreader_tw_locked_value == True and 0 < trolley_position < 3.5 and hoist_position > 7.5:
		try:
			JobStepping.get_tts_close_to_rtg.main(latitude,longitude,rtg_name,"Lift container",spreader)
		except Exception as e:
			logger.info('Error ejecutando JobSteppingPointInCircle en Tag Change %s')%e
			
	if  spreader_tw_locked_value == True and 3.5 < trolley_position < 20:
		try:
			JobStepping.rtg_stack_events.main(rtg_name,trolley_position,hoist_position,"Lift container inside the block",spreader)
		except Exception as e:
			logger.info('Error ejecutando JobSteppingPointInCircle lift the container inside the block en Tag Change %s'%e)
